    ...
    def evaluate(self, test_config):
        ...
        with torch.no_grad():
            ...
            for it, eval_data in enumerate(self.data_loaders["test"]):
                ...
                a1 = np.array(copy.deepcopy(eval_data['root_files'])).astype(str)
                a2 = np.array(['_' for i in range(len(a1))])
                a3 = np.char.mod('%d', copy.deepcopy(eval_data['event_ids']))

                b1  = np.core.defchararray.add(a1, a2)
                eval_ids = np.core.defchararray.add(b1, a3)
